...
module.exports = {
	...
	consolidate: function(data){
		...
		if(data.total == undefined){
			...
			consolidated.icon = consolidated.icon = fs.readFileSync(path.join(__dirname, '..', 'resources', 'icons', 'info.png'));
		} else {
			...
			if(coefficient == 100){
				consolidated.icon = fs.readFileSync(path.join(__dirname, '..', 'resources', 'icons', 'success.png'));
			} else {
				consolidated.icon = fs.readFileSync(path.join(__dirname, '..', 'resources', 'icons', 'error.png'));
			}
		}
